      ...
      <div class="perf-body">
        ...
        <div class="perf-left">
          ...
          <NGrid :x-gap="14" :y-gap="14" :cols="3" responsive="screen" item-responsive>
              ...
              <NCard class="panel-card panel-card--overview" :bordered="true" size="small">
                <div class="card-body">
                  <div class="card-header">
                    <SvgIcon icon="heroicons:cpu-chip" class="card-icon cpu-color" /><span class="card-title">CPU</span>
                  </div>
                  <div class="big-value" :style="{ color: usageColor(stats.cpu.usage) }">{{ stats.cpu.usage }}<span
                      class="unit">%</span></div>
                  <!-- 型号 -->
                  <div class="info-row">{{ stats.cpu.model || '—' }}</div>
                  <!-- 详情 -->
                  <div class="detail-list">
                    <div class="detail-row"><span>{{ $t('perfView.physCores') }}</span><b>{{ stats.cpu.physicalCores
                        }}</b><span class="sep">|</span><span>{{ $t('perfView.logiCores') }}</span><b>{{
                          stats.cpu.logicalCores }}</b></div>
                    <div class="detail-row"><span>{{ $t('perfView.baseClock') }}</span><b>{{
                      formatGhz(stats.cpu.baseClock) }} GHz</b><span class="sep">|</span><span>{{
                          $t('perfView.curClock') }}</span><b>{{ formatGhz(stats.cpu.speed) }} GHz</b></div>
                    <div class="detail-row"><span>{{ $t('perfView.avgClock') }}</span><b>{{ formatGhz(avgCoreFreq) }}
                        GHz</b><span class="sep">|</span><span>L2</span><b>{{ formatKb(stats.cpu.l2Cache) }}</b></div>
                    <div class="detail-row" v-if="stats.cpu.l3Cache"><span>L3 Cache</span><b>{{
                      formatKb(stats.cpu.l3Cache) }}</b></div>
                  </div>
                </div>
              </NCard>
            </NGridItem>
            ...
            <NGridItem span="3 s:3 m:1 l:1">
              <NCard class="panel-card panel-card--overview" :bordered="true" size="small">
                <div class="card-body">
                  <div class="card-header">
                    <SvgIcon icon="mdi:memory" class="card-icon ram-color" /><span class="card-title">{{
                      $t('perfView.memory') }}</span>
                  </div>
                  <div class="big-value" :style="{ color: usageColor(stats.memory.usagePercent) }">{{
                    stats.memory.usagePercent }}<span class="unit">%</span></div>
                  <div class="info-row">{{ formatBytes(stats.memory.used) }} / {{ formatBytes(stats.memory.total) }}
                  </div>
                  <div class="detail-list">
                    <div class="detail-row"><span>{{ $t('perfView.available') }}</span><b>{{
                      formatBytesCompact(stats.memory.free) }}</b></div>
                    <div class="detail-row" v-if="stats.memory.swapTotal"><span>Swap</span><b>{{
                      stats.memory.swapUsagePercent }}%</b><span class="sep">|</span><span>{{ $t('perfView.swapUsed')
                        }}</span><b>{{ formatBytesCompact(stats.memory.swapUsed) }} / {{
                          formatBytesCompact(stats.memory.swapTotal) }}</b></div>
                    <div class="detail-row" v-if="hasVirtualMemory"><span>{{ $t('perfView.virtualMem') }}</span><b>{{
                      formatBytesCompact(stats.memory.virtualTotal - stats.memory.virtualFree) }} / {{
                          formatBytesCompact(stats.memory.virtualTotal) }}</b></div>
                  </div>
                  <!-- 内存条 -->
                  <div v-if="effectiveShowSticks" class="sticks">
                    <div class="sticks-title">{{ $t('perfView.memSticks') }} <span class="sticks-count">{{
                      stats.memory.sticks.length }}</span></div>
            ...
            <NGridItem span="3 s:3 m:1 l:1">
              <NCard class="panel-card panel-card--overview" :bordered="true" size="small">
                <div class="card-body">
                  <div class="card-header">
                    <SvgIcon icon="mdi:expansion-card" class="card-icon gpu-color" /><span class="card-title">GPU</span>
                  </div>
                  <template v-if="hasGpuUsage">
                    <div class="big-value" :style="{ color: usageColor(stats.gpu.usagePercent!) }">{{
                      stats.gpu.usagePercent }}<span class="unit">%</span></div>
                  </template>
                  <template v-else>
                    <div class="gpu-fallback-name">{{ stats.gpu.model || $t('perfView.noData') }}</div>
                  </template>
                  <!-- 温度 -->
                  <div class="temp-row" v-if="effectiveHasGpuTemp">
                    <SvgIcon icon="mdi:thermometer" class="temp-icon"
                      :style="{ color: tempColor(stats.gpu.temperature!) }" /><span class="temp-value"
                      :style="{ color: tempColor(stats.gpu.temperature!) }">{{ stats.gpu.temperature }}°C</span>
                  </div>
                  <!-- 显存 -->
                  <div v-if="hasGpuMem" class="vram">
                    ...
                    <div class="vram-row">
                      <div class="vram-bar">
                        <div class="vram-fill"
                          :style="{ width: gpuMemPercent + '%', backgroundColor: usageColor(gpuMemPercent) }" />
                      </div>
                      <span class="vram-text">{{ formatBytesCompact(stats.gpu.memoryUsed!) }} / {{
                        formatBytesCompact(stats.gpu.memoryTotal!) }}</span>
                    </div>
                  </div>
                  <!-- GPU 传感器 -->
                  <div v-if="effectiveShowGpuSensors" class="gpu-sensors">
                    <div class="sensor" v-if="stats.gpu.coreClock !== null">
                      <SvgIcon icon="mdi:sine-wave" class="sensor-icon" /><span>{{ $t('perfView.gpuCore') }}</span><b>{{
                        formatGhz(stats.gpu.coreClock!) }} GHz</b>
                    </div>
                    <div class="sensor" v-if="stats.gpu.memClock !== null">
                      <SvgIcon icon="mdi:memory" class="sensor-icon" /><span>{{ $t('perfView.gpuMem') }}</span><b>{{
                        formatGhz(stats.gpu.memClock!) }} GHz</b>
                    </div>
                    <div class="sensor" v-if="stats.gpu.powerDraw !== null">
                      <SvgIcon icon="mdi:flash" class="sensor-icon" /><span>{{ $t('perfView.gpuPower') }}</span><b
                        :style="{ color: usageColor(stats.gpu.powerDraw) }">{{ stats.gpu.powerDraw }} W</b>
                    </div>
                    <div class="sensor" v-if="stats.gpu.fanSpeed !== null">
                      <SvgIcon icon="mdi:fan" class="sensor-icon" /><span>{{ $t('perfView.gpuFan') }}</span><b>{{
                        stats.gpu.fanSpeed
                        }}%</b>
                    </div>
                  </div>
                  <!-- GPU 信息 -->
                  <div class="detail-list">
                    <div class="detail-row" v-if="stats.gpu.model"><span>{{ $t('perfView.gpuModel') }}</span><b>{{
                        stats.gpu.model }}</b>
                    </div>
                    <div class="detail-row" v-if="stats.gpu.adapterRam"><span>{{ $t('perfView.vram') }}</span><b>{{
                      formatBytes(stats.gpu.adapterRam) }}</b></div>
                    <div class="detail-row" v-if="stats.gpu.driverVersion"><span>{{ $t('perfView.gpuDriver')
                        }}</span><b>{{
                          stats.gpu.driverVersion }}</b></div>
                  </div>
                </div>
              </NCard>
            </NGridItem>
          ...
          <NGrid :x-gap="14" :y-gap="14" :cols="2" responsive="screen" item-responsive>
            <NGridItem span="2 s:1 m:1 l:1" v-if="effectiveShowProcess">
              <NCard class="panel-card panel-card--info" :bordered="true" size="small">
                <div class="card-body">
                  <div class="card-header">
                    <SvgIcon icon="mdi:application-brackets" class="card-icon proc-color" /><span class="card-title">{{
                      $t('perfView.process') }}</span>
                  </div>
                  <div class="detail-list">
                    <div class="detail-row"><span>{{ $t('perfView.procMemory') }}</span><b>{{
                      formatBytesCompact(stats.process.memoryUsage) }}</b></div>
                    <div class="detail-row"><span>{{ $t('perfView.procHeap') }}</span><b>{{
                      formatBytesCompact(stats.process.heapUsed) }} / {{ formatBytesCompact(stats.process.heapTotal)
                        }}</b>
                    </div>
                    <div class="detail-row"><span>{{ $t('perfView.procCpu') }}</span><b
                        :style="{ color: usageColor(stats.process.cpuUsage) }">{{ stats.process.cpuUsage }}%</b></div>
                  </div>
                </div>
              </NCard>
            </NGridItem>
            <NGridItem span="2 s:1 m:1 l:1" v-if="effectiveShowSystem">
              <NCard class="panel-card panel-card--info" :bordered="true" size="small">
                <div class="card-body">
                  <div class="card-header">
                    <SvgIcon icon="mdi:desktop-tower-monitor" class="card-icon sys-color" /><span class="card-title">{{
                      $t('perfView.systemInfo') }}</span>
                  </div>
                  <div class="detail-list">
                    <div class="detail-row"><span>{{ $t('perfView.hostname') }}</span><b>{{ stats.system.hostname || '—'
                        }}</b>
                    </div>
                    <div class="detail-row"><span>{{ $t('perfView.os') }}</span><b>{{ stats.system.platform }} {{
                        stats.system.arch }}</b></div>
                    <div class="detail-row"><span>{{ $t('perfView.uptime') }}</span><b>{{
                      formatUptime(stats.system.uptime) }}</b>
                    </div>
                    <div class="detail-row"><span>{{ $t('perfView.totalRam') }}</span><b>{{
                      formatBytes(stats.system.totalMem)
                        }}</b></div>
                  </div>
                </div>
              </NCard>
          ...
          <NCard class="panel-card panel-card--cores" :bordered="true" size="small">
            <div class="card-body">
              <div class="card-header">
                <SvgIcon icon="heroicons:cpu-chip" class="card-icon cpu-color" /><span class="card-title">{{
                  $t('perfView.cores')
                  }}</span>
              </div>
          ...
          <NCard class="config-card" :bordered="true" size="small">
            ...
            <div class="config-group">
              ...
              <div class="config-options">
                <button v-for="opt in INTERVAL_OPTIONS" :key="opt.value" class="config-btn"
                  :class="{ active: pollInterval === opt.value }" @click="store.applyPollInterval(opt.value)">{{
                  opt.label
                  }}</button>
              </div>
            </div>

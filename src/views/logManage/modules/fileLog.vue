...
import { fetchGetMonLogsFilePageList, fetchRemoveMonLogsFile, fetchClearMonLogsFile } from '@/service/api';
import { $t } from '@/locales';
import SvgIcon from '@/components/custom/svg-icon.vue';

defineOptions({ name: 'FileLog' });

/** 清空确认弹窗中的日志类型名称（i18n key） */
const MODULE_LABEL_KEY = 'logManage.tabs.file';
...
const loadData = async () => {
  ...
  try {
    const { data, error } = await fetchGetMonLogsFilePageList({
      userName: pagination.keyword ? Number(pagination.keyword) : undefined,
...
const handleConfirmDelete = async () => {
  ...
  try {
    const { error } = await fetchRemoveMonLogsFile(currentDeleteId.value);
    if (error) {
...
const handleConfirmClear = async () => {
  ...
  try {
    const { error } = await fetchClearMonLogsFile();
    if (error) {
...
/** 文件大小：等宽字体展示 */
const renderSizeCell = (row: Record<string, any>) => h('span', { class: 'col-size' }, row.fileSize ?? '-');

/** 文件路径：图片缩略图 / 文件图标 + 文本 */
const renderFileUrlCell = (row: Record<string, any>) =>
  h('div', { class: 'file-url-cell' }, [
    isImageFile(row.fileUrl)
      ? h(NImage, {
          src: row.fileUrl,
          class: 'file-thumb',
          width: 34,
      ...
      : h(SvgIcon, { icon: 'mdi:file-document-outline', class: 'file-type-icon' }),
    h('span', { class: 'cell-text' }, [
      row.fileUrl != null && row.fileUrl !== ''
        ? h(
            NEllipsis,
            { maxLine: 1, tooltip: { placement: 'top' } },
            { default: () => String(row.fileUrl) }
          )
        : '-'
    ])
  ]);
...
const renderStatusCell = (row: Record<string, any>) =>
  h('span', { class: `status-badge${isLoginSuccess(row.status) ? ' success' : ''}` }, [
    h('span', { class: 'dot' }),
...
const columns = computed<DataTableColumns<Record<string, any>>>(() => [
  {
    key: 'userId',
    title: $t('logManage.table.userId'),
    width: resizedWidths.value['userId'] ?? 90,
    minWidth: 70,
    resizable: true,
    render: row => renderTextCell(row.userId)
  },
  {
    key: 'userName',
    title: $t('logManage.table.userName'),
    width: resizedWidths.value['userName'] ?? 110,
    minWidth: 90,
    resizable: true,
  ...
  {
    key: 'fileUrl',
    title: $t('logManage.table.fileUrl'),
    minWidth: 180,
    resizable: true,
    width: resizedWidths.value['fileUrl'],
    render: row => renderFileUrlCell(row)
  },
  {
    key: 'fileSize',
    title: $t('logManage.table.fileSize'),
    width: resizedWidths.value['fileSize'] ?? 90,
    minWidth: 70,
    resizable: true,
    render: row => renderSizeCell(row)
  },
  {
    key: 'status',
    title: $t('logManage.table.status'),
    width: resizedWidths.value['status'] ?? 110,
    minWidth: 90,
    resizable: true,

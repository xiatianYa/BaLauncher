...
import { fetchGetMonLogsOperationPageList, fetchRemoveMonLogsOperation, fetchClearMonLogsOperation } from '@/service/api';
...
const MODULE_LABEL_KEY = 'logManage.tabs.operation';
...
const loadData = async () => {
  ...
  try {
    const { data, error } = await fetchGetMonLogsOperationPageList({
      createUser: pagination.keyword ? Number(pagination.keyword) : undefined,
...
const handleConfirmDelete = async () => {
  ...
  try {
    const { error } = await fetchRemoveMonLogsOperation(currentDeleteId.value);
    if (error) {
...
const handleConfirmClear = async () => {
  ...
  try {
    const { error } = await fetchClearMonLogsOperation();
    if (error) {
...
/** 请求方式：语义色徽章（颜色由行内 style 注入） */
const renderMethodCell = (row: Record<string, any>) => {
  const method = row.requestMethod;
  const color = getMethodColor(method);
  return h(
    'span',
    { class: 'method-badge', style: { color, borderColor: `${color}55`, background: `${color}14` } },
    method || '-'
  );
};

/** 耗时：等宽字体毫秒展示 */
const renderCostCell = (row: Record<string, any>) =>
  h('span', { class: 'col-cost' }, row.useTime != null ? `${row.useTime}ms` : '-');
...
const columns = computed<DataTableColumns<Record<string, any>>>(() => [
  {
    key: 'operation',
    title: $t('logManage.table.operation'),
    minWidth: 140,
    resizable: true,
    width: resizedWidths.value['operation'],
    render: row => renderTextCell(row.operation)
  },
  {
    key: 'requestMethod',
    title: $t('logManage.table.requestMethod'),
    width: resizedWidths.value['requestMethod'] ?? 90,
    minWidth: 80,
    resizable: true,
    render: row => renderMethodCell(row)
  },
  {
    key: 'requestUri',
    title: $t('logManage.table.requestUri'),
    minWidth: 160,
    resizable: true,
    width: resizedWidths.value['requestUri'],
    render: row => renderTextCell(row.requestUri)
  },
  {
    key: 'createUser',
    title: $t('logManage.table.createUser'),
    width: resizedWidths.value['createUser'] ?? 110,
    minWidth: 90,
    resizable: true,
    render: row => renderTextCell(row.createUser)
  },
  {
    key: 'ip',
  ...
  {
    key: 'useTime',
    title: $t('logManage.table.useTime'),
    width: resizedWidths.value['useTime'] ?? 80,
    minWidth: 70,
    resizable: true,
    render: row => renderCostCell(row)
  },
  {
    key: 'createTime',
...
<template>
  ...
  <div class="search-bar">
    <div class="search-box">
      ...
      <NInput v-model:value="pagination.keyword" :placeholder="$t('logManage.search.operationPlaceholder')" clearable size="small" />
    </div>

...
<template>
  ...
  <div class="log-table">
    ...
    <div class="log-table-body">
      <div v-for="row in list" :key="row.id" class="log-row">
        <span class="col-operation cell-text" :title="row.operation">{{ row.operation || '-' }}</span>
        <span class="col-method">
          <span class="method-badge"
            :style="{ color: getMethodColor(row.requestMethod), borderColor: `${getMethodColor(row.requestMethod)}55`, background: `${getMethodColor(row.requestMethod)}14` }">
            {{ row.requestMethod || '-' }}
          </span>
        </span>

...
import { fetchGetMonLogsErrorPageList, fetchRemoveMonLogsError, fetchClearMonLogsError } from '@/service/api';
...
const MODULE_LABEL_KEY = 'logManage.tabs.error';
...
const handleConfirmDelete = async () => {
  ...
  try {
    const { error } = await fetchRemoveMonLogsError(currentDeleteId.value);
    if (error) {
...
const handleConfirmClear = async () => {
  ...
  try {
    const { error } = await fetchClearMonLogsError();
    if (error) {
...
/** 异常行号：等宽字体展示 */
const renderLineCell = (row: Record<string, any>) => h('span', { class: 'col-line' }, row.line ?? '-');
...
const columns = computed<DataTableColumns<Record<string, any>>>(() => [
  ...
  },
  {
    ...
    title: $t('logManage.table.exceptionMessage'),
    minWidth: 150,
    resizable: true,
    width: resizedWidths.value['exceptionMessage'],
    render: row => renderTextCell(row.exceptionMessage)
  },
  {
    key: 'exceptionClass',
    title: $t('logManage.table.exceptionClass'),
    width: resizedWidths.value['exceptionClass'] ?? 170,
    minWidth: 140,
    resizable: true,
    render: row => renderTextCell(row.exceptionClass)
  },
  {
    key: 'line',
    title: $t('logManage.table.line'),
    width: resizedWidths.value['line'] ?? 70,
    minWidth: 60,
    resizable: true,
    render: row => renderLineCell(row)
  },
  {
    key: 'createUser',
